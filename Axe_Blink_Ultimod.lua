...
local blademails = drawMgr:CreateRect(-5,-70,35,20,0x000000ff) blademails.visible = false
function Tick(tick)
	...
	blademails.entity = me
	blademails.entityPosition = Vector(0,0,me.healthbarOffset)
	damage = {250,325,400}
	adamage = {300,425,550}
	
	local Cullblade = me:GetAbility(4)
	local Blink = me:FindItem("item_blink")
	local BladeMails = me:FindItem("item_blade_mail")
	local Crimsons = me:FindItem("item_crimson_guard")
	...
	if Cullblade.level > 0 or call.level > 0 then
		...
		if BladeMails then
			blademails.textureId = drawMgr:GetTextureId("NyanUI/items/blade_mail")
			blademails.visible = active
		else
			blademails.visible = false
		end
							...
							elseif callactive then
								...
								if not v:IsInvul() and GetDistance2D(v,me)-25 <= call:GetSpecialData("radius",call.level) and ((pred and GetDistance2D(pred,me)-25 <= call:GetSpecialData("radius",call.level)) or not pred) then
									if SleepCheck("call") then
										...
										if Pipes then
										me:SafeCastItem("item_pipe")
										end
										if Shivas then
										me:SafeCastItem("item_shivas_guard")
										end
										if Bucklers then
