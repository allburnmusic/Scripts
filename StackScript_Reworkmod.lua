--<<Stacks ancient camps and allows to stack enemy ancients>>
--===Rework By Blaxpirit modded by d000000h===-- 
...
config:SetParameter("RapeEnemy", "K",config.TYPE_HOTKEY )
config:Load()

hotkey = config.Hotkey
settozero = config.SetQuantityToZero
increasenumber = config.IncreaseNumber
rapeenemy = config.RapeEnemy
x,y = 5, 50
...
function Key(msg,code)
	...
	if code == rapeenemy then
		if rape then
		rape = false
		text.text = "StackScript: Rape the enemy ancients now inactive."
		else
		text.text = "StackScript: Rape the enemy ancients now active."
		rape = true
		end
	end
